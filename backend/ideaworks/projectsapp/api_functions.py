...
def tag_based_filtering(request, data):
    ...
    # Sort the projects in reverse order based on the number of matching tags
    sorted_data = sorted(object_holder_list, key=lambda k: k['tags_matched'], reverse=True)
    
    # Return just the project objects
    data['objects'] = [sorted_obj['bundle_obj'] for sorted_obj in sorted_data]
...
def get_user_vote_status(bundle):
    ...
    # Has the current user voted on this project?
    pos_votes = [vote.data['user'] for vote in bundle.data['backs']]
    
    if unicode(bundle.request.user) in pos_votes:
        user_vote = 1
    else:
        user_vote = 0
    return user_vote
    ...
    if data_level == 'meta':
        ...
        for record in response_data['objects']:
            for key in record.data.keys():
                ...
                if key.lower() not in RESPONSE_FIELDS[data_level]: 
                    del record.data[key]
...
def merge_tag_results(proj_res, idea_res):
    """
    Merges tags from 2 distinct aggregation calls.
    """
    # Placeholder for results output
    output = []
    
    # Reformat the data structure
    # ** THIS IS PYTHON 2.7 SYNTAX **
    #proj_tags = {tag['_id'] : tag['count'] for tag in proj_res}
    #idea_tags = {tag['_id'] : tag['count'] for tag in idea_res}
    
    # ** TO ALLOW PYTHON 2.6 SYNTAX
    proj_tags = {}
    for tag in proj_res:
        proj_tags[tag['_id']] = tag['count']
    idea_tags = {}
    for tag in idea_res:
        idea_tags[tag['_id']] = tag['count']
        
    # Get all keys - distinctify them
    keys = list(set(proj_tags.keys() + idea_tags.keys()))
    
    # Loop the keys and access the count
    for key in keys:
        
        # Get counts from projects and from ideas
        try:    proj_val = proj_tags[key]
        except: proj_val = 0
        try:    idea_val = idea_tags[key]
        except: idea_val = 0
        
        # Sum and add to output
        val = proj_val + idea_val
        output.append({"_id":key, "count":val})

    return sorted(output, key=lambda k: k['count'], reverse=True)
